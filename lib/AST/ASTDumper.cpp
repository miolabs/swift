...
std::unordered_map<std::string, std::string> functionUniqueNames = {
  ...
  {"Swift.(file).min(_:T,_:T,_:T,_:[T])", "minMany"},
  {"Swift.(file).max(_:T,_:T,_:T,_:[T])", "maxMany"}
};
...
namespace {
  ...
  public:
    ...
    void visitSourceFile(const SourceFile &SF) {
      ...
      if(LIB_GENERATE_MODE) {
        for(int i = 0; i < 3; i++) {
          std::ifstream infile(i == 2 ? LIB_BODIES_PATH_AUTOGENERATED : i ? LIB_BODIES_PATH_MANUAL_SWIFT : LIB_BODIES_PATH_MANUAL);
...
public:
  ...
  std::string handleInfo(std::string lrString, Expr *lExpr, bool isSubscriptAss = false) {
    ...
    lDeclrefExpr->getDeclRef().isSpecialized()
