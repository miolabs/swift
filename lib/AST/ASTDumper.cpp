...
std::unordered_map<std::string, std::string> functionUniqueNames = {
  ...
  {"Swift.(file).MutableCollection.subscript(_:Range<Self.Index>)", "subscriptRange"},
  {"Swift.(file).BidirectionalCollection.subscript(_:Range<Self.Index>)", "subscriptRange"},
  {"Swift.(file).Collection.subscript(_:Range<Self.Index>)", "subscriptRange"},
  {"Swift.(file).RangeReplaceableCollection.subscript(_:Range<Self.Index>)", "subscriptRange"},
...
std::unordered_map<std::string, std::string> nameReplacementsById = {};
...
std::string getName(ValueDecl *D, unsigned long satisfiedProtocolRequirementI = 0) {
  ...
  if(auto *functionDecl = dyn_cast<AbstractFunctionDecl>(D)) {
    name = getFunctionName(functionDecl, uniqueIdentifier);
  }
  else if(auto *subscriptDecl = dyn_cast<SubscriptDecl>(D)) {
    name = getFunctionName(subscriptDecl, uniqueIdentifier);
  }
  else if(D->hasName()) {
  ...
  if(nameReplacementsById.count(uniqueIdentifier)) {
    return nameReplacementsById[uniqueIdentifier];
  }
...
namespace {
  ...
  public:
    ...
    void visitAnyStructDecl(std::string kind, NominalTypeDecl *D) {
      ...
      if(LIB_GENERATE_MODE && LIB_ADDITIONAL_BODY.count(getMemberIdentifier(D))) {
        OS << "\n" << LIB_ADDITIONAL_BODY.at(getMemberIdentifier(D));
      }
    ...
    if (auto lConstructor = dyn_cast<ConstructorRefCallExpr>(lExpr)) {
      if (auto initDeclRef = dyn_cast<DeclRefExpr>(lConstructor->getFn())) {
        if (auto initDecl = dyn_cast<ConstructorDecl>(initDeclRef->getDecl())) {
          ...
          else if(getMemberIdentifier(initDecl) == "Swift.(file)._OptionalNilComparisonType.init(nilLiteral:())") {
            lString = "null";
          }
          else {
